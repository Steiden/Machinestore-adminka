...
try {
  ...
  inputs.forEach((input) => {
    ...
    inputField.addEventListener("input", (e) => {
      ...
      if (inputField.value) input.classList.add("d-input--active"); else input.classList.remove("d-input--active");
    });
  });
} catch (e) {
  ...
  async function initMap() {
    ...
    yandexMapContainers?.forEach((yandexMapContainer) => {
      ...
      markerElement.addEventListener("click", (e) => {
        ...
        markerContent.classList.contains("marker__content--visible") ? markerContent.classList.remove("marker__content--visible") : markerText.textContent && markerContent.classList.add("marker__content--visible");
      });
      ...
      const mapListener = new YMapListener({
        layer: "any", onClick: (object, event) => {
      ...
      const map = new YMap(yandexMapContainer, {
        location: {
          center: [37.588144, 55.733842], zoom: 10,
        }, theme: "dark", showScaleInCopyrights: true,
      });
...
try {
  ...
  window.addEventListener("click", (e) => {
    let isAvailable = Array.from(catalogs).reduce((result, catalog) => !catalog.contains(e.target) && result, true) && Array.from(catalogToggles).reduce((result, toggle) => !toggle.contains(e.target) & result, true);
...
try {
  ...
  sheets.forEach((sheet) => {
    ...
    const triggers = document.querySelectorAll(`[data-sheet-trigger=${sheet.getAttribute("data-sheet")}`);
    triggers.forEach((trigger) => trigger.addEventListener("click", () => sheet.classList.add("d-sheet--active")));
  });
} catch (e) {
