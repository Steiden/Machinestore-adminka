...
try {
	// Функционал работы аккордеона

	const accordions = document.querySelectorAll("[data-accordion]");
	accordions.forEach((accordion) => {
		accordion.addEventListener("click", () => {
			const selector = accordion.getAttribute("data-accordion");
			accordion.classList.toggle(`${selector}--expanded`);
		});
	});
} catch (e) {
	console.error("Ошибка работы аккордеона: " + e);
}
